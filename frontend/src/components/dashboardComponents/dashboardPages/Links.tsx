      ...
      <div>
        {
          ...
          linksContent.filter((el) => el.contentType === "Link").map((el) => (
            <div key={el._id}>
              <GridContent contentType={el?.contentType} title={el.title} description={el.description} type={el.type} createdAt={el.createdAt} link={el.link} image={""} contentShowType={"rows"} id={el._id}/>
            </div>
          ))
        }
      </div>
